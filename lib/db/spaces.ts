 ...
 *
 * @example
 * const space = await getSpace(supabase, 'space-uuid');
 */
 ...
 *
 * @example
 * const spaces = await listSpaces(supabase, 'venue-uuid');
 */
export async function listSpaces(
  ...
  return data?.map((space: any) => SpaceSchema.parse(space)) || [];
}
 ...
 *
 * @example
 * const newSpace = await createSpace(supabase, {
 *   venue_id: 'venue-uuid',
 *   name: 'Grand Ballroom',
 ...
 *
 * @example
 * const updated = await updateSpace(supabase, 'space-uuid', {
 *   capacity: 350,
 ...
 *
 * @example
 * await deleteSpace(supabase, 'space-uuid');
 */
 ...
 *
 * @example
 * await addSpacePhotos(supabase, 'space-uuid', [
...
export async function addSpacePhotos(
  ...
  const space = await getSpace(supabase, space_id);
  if (!space) {
  ...
  return updateSpace(supabase, space_id, { photos: updatedPhotos });
}
 ...
 *
 * @example
 * await removeSpacePhoto(supabase, 'space-uuid', 'https://old-photo.jpg');
 */
 ...
 *
 * @example
 * const available = await checkSpaceAvailability(supabase, 'space-uuid', '2025-06-15T14:00:00Z');
 */
 ...
 *
 * @example
 * const upcomingEvents = await getSpaceUpcomingEvents(supabase, 'space-uuid');
 */
